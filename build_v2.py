...
# Step 1: Add MediaPipe scripts after Three.js script
three_js_script = '<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>'
mediapipe_scripts = '''<script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/@mediapipe/camera_utils/camera_utils.js" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/@mediapipe/control_utils/control_utils.js" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/@mediapipe/drawing_utils/drawing_utils.js" crossorigin="anonymous"></script>
    <script src="https://cdn.jsdelivr.net/npm/@mediapipe/hands/hands.js" crossorigin="anonymous"></script>'''

original = original.replace(three_js_script, mediapipe_scripts, 1)

# Step 2: Add scan-line, corner-frames, webcam, hand-canvas, HUD before content-layer
content_layer_div = '<div class="content-layer">'
...
original = original.replace(content_layer_div, new_elements, 1)

# Step 3: Add new CSS for the new elements
style_end = '    </style>'
...
original = original.replace(style_end, new_css, 1)

print(f"After CSS/HTML updates: {len(original)} chars")

# Step 4: Replace the entire <script> block with new Three.js + Hand tracking code
...
if script_start > 0 and script_end > script_start:
    ...
    new_js = '''    <script>
        ...
        function createEnvironment() {
            const ring = new THREE.Mesh(new THREE.RingGeometry(8, 12, 64), new THREE.MeshBasicMaterial({color: 0x6ab4ff, transparent: true, opacity: 0.1, side: THREE.DoubleSide}));
            ring.rotation.x = -Math.PI / 2; ring.position.y = -8; scene.add(ring);
            for (let i = 0; i < 5; i++) {
                const rMesh = new THREE.Mesh(new THREE.TorusGeometry(3+i*2, 0.03, 8, 64), new THREE.MeshBasicMaterial({color: i%2===0?0x6ab4ff:0xffd700, transparent: true, opacity: 0.15}));
                rMesh.rotation.x = Math.PI/2+(Math.random()-0.5)*0.5; rMesh.position.y = -5+i*3; rMesh.userData = {rotationSpeed: 0.002*(i%2===0?1:-1)}; scene.add(rMesh);
            }
        ...
        function focusPhoto(index) {
            if (index < 0 || index >= photoMeshes.length) return; selectedPhotoIndex = index; targetState = STATE.PHOTO_FOCUS;
            const img = document.getElementById('focusImage'); img.src = photos[index]; document.getElementById('photoFocus').classList.add('show');
        }
        ...
        function processGesture(gesture) {
            ...
            if (gesture.extendedFingers === 0) { document.getElementById('gest-fist').classList.add('active'); targetState = STATE.COLLAPSED; gestureHoldTime++; }
                ...
                if (currentState === STATE.SCATTERED || currentState === STATE.PHOTO_FOCUS) { const handX = gesture.landmarks[9].x, handY = gesture.landmarks[9].y; targetCameraAngleY = (0.5 - handX) * Math.PI; targetCameraAngleX = (0.5 - handY) * Math.PI * 0.5; }
            } else if (gesture.extendedFingers >= 2 && gesture.extendedFingers < 4) {
                document.getElementById('gest-rotate').classList.add('active');
                const handX = gesture.landmarks[9].x, handY = gesture.landmarks[9].y; targetCameraAngleY += (0.5 - handX) * 0.02; targetCameraAngleX += (0.5 - handY) * 0.01;
            }
        }
        ...
        function updateParticles(time, deltaTime) {
            ...
            for (let i = 0; i < particleCount; i++) {
                ...
                const targetX = orig.x * (1 - easeT) + scatter.x * easeT, targetY = orig.y * (1 - easeT) + scatter.y * easeT, targetZ = orig.z * (1 - easeT) + scatter.z * easeT;
                const floatY = Math.sin(time * 0.5 + i * 0.1) * 0.3 * easeT, floatX = Math.cos(time * 0.3 + i * 0.15) * 0.2 * easeT;
                positions[i*3] += (targetX + floatX - positions[i*3]) * 0.05; positions[i*3+1] += (targetY + floatY - positions[i*3+1]) * 0.05; positions[i*3+2] += (targetZ - positions[i*3+2]) * 0.05;
            }
        ...
        function updateDataElements(time, deltaTime) {
            ...
            dataElements.forEach((mesh) => {
                const data = mesh.userData; if (currentState === STATE.PHOTO_FOCUS && selectedPhotoIndex >= 0) return;
                const targetX = data.originalPos.x * (1 - easeT) + data.scatteredPos.x * easeT, targetY = data.originalPos.y * (1 - easeT) + data.scatteredPos.y * easeT, targetZ = data.originalPos.z * (1 - easeT) + data.scatteredPos.z * easeT;
                ...
                const targetRotX = data.originalRot.x * (1 - easeT) + data.scatteredRot.x * easeT, targetRotY = data.originalRot.y * (1 - easeT) + data.scatteredRot.y * easeT, targetRotZ = data.originalRot.z * (1 - easeT) + data.scatteredRot.z * easeT;
                mesh.rotation.x += (targetRotX - mesh.rotation.x) * 0.03 + data.rotationSpeed.x * easeT; mesh.rotation.y += (targetRotY - mesh.rotation.y) * 0.03 + data.rotationSpeed.y * easeT; mesh.rotation.z += (targetRotZ - mesh.rotation.z) * 0.03 + data.rotationSpeed.z * easeT;
            });
        }
        ...
        function updatePhotoMeshes(time, deltaTime) {
            ...
            photoMeshes.forEach((mesh, i) => {
                ...
                if (currentState === STATE.PHOTO_FOCUS && i === selectedPhotoIndex) {
                    ...
                    mesh.rotation.x += (0 - mesh.rotation.x) * 0.05; mesh.rotation.y += (0 - mesh.rotation.y) * 0.05; mesh.rotation.z += (0 - mesh.rotation.z) * 0.05; mesh.scale.setScalar(1 + (3 - 1) * 0.05); mesh.material.opacity = 1;
        ...
        document.addEventListener('keydown', (e) => { switch(e.key) { case '1': targetState = STATE.COLLAPSED; break; case '2': targetState = STATE.SCATTERED; break; case '3': if (photoMeshes.length > 0) focusPhoto(0); break; case 'Escape': closePhotoFocus(); closePhotoPanel(); break; case 'u': case 'U': document.getElementById('photoPanel').classList.toggle('show'); break; } });
    ...
    original = original[:script_start] + new_js

with open(r'C:\Users\AQQW\Desktop\index.html', 'w', encoding='utf-8') as f:
    f.write(original)

print(f"Done! File size: {len(original)} chars")
